    ...
    try:
        ...
        for i in range(len(epochs)):
            ...
            if len(idx) > 0:
                ...
                labels_for_epochs.append(label_map.get(annotations.description[idx[-1]], 5)) 
            else:
                labels_for_epochs.append(5) 
...
if __name__ == "__main__":
    ...
    all_test_probs_clean, all_test_probs_full = [], [] 
    all_histories = []
    total_time = 0.0
    ...
    for i, (train_idx, test_idx) in enumerate(gkf.split(range(len(unique_subjects)), groups=unique_subjects)):
        print(f"\n===== 🚀 Fold {i+1}/{CONFIG.N_FOLDS} ====="); start_time = time.time()
        ...
        alpha_vals = inv_freq / np.mean(inv_freq)  
        ...
        l2_logger = L2LossLogger()       

        
        cbs = [
            clean_f1_callback, 
            SpeedLogger(), metrics_logger, l2_logger 
        ]
        ...
        all_histories[-1]['lr'] = metrics_logger.learning_rates 
        all_histories[-1]['l2_loss'] = l2_logger.l2_losses 
        ...
        if len(y_test_clean) > 0:
            ...
            all_test_probs_clean.append(y_pred_probs_clean) 
        else:
        ...
        all_test_probs_full.append(y_pred_probs_full) 
    ...
    BASE_RESULTS_DIR = f"results_lstm_6lop_{TIMESTAMP}"
    ...
    if os.path.exists(cache_path):
        try:
            ...
            all_test_probs_clean = cache.get("all_test_probs_clean", []) 
            all_test_probs_full = cache.get("all_test_probs_full", [])   
    ...
    report_clean = classification_report(
        ...
        y_pred_clean_final,
        labels=list(range(5)),   
        target_names=CONFIG.SLEEP_STAGE_LABELS[:-1],  
        digits=4,
        zero_division=0
    )
    ...
    if y_probs_full_all.any():
        ...
        plt.xlabel("Xác suất dự đoán cao nhất"); plt.ylabel("Số lượng mẫu")
    ...
    # ================== Lưu cache ==================
    joblib.dump({ "fold_results": fold_results, 
        "all_test_true_clean": all_test_true_clean,
        "all_test_preds_clean": all_test_preds_clean,
        "all_test_probs_clean": all_test_probs_clean, 
        "all_test_true_full": all_test_true, 
        "all_test_preds_full": all_test_preds, 
        "all_test_probs_full": all_test_probs_full, 
...
def predict_subject_from_saved_model(model_path, subject_id, apply_hmm=True, collapse_threshold=0.90, temp=1.0, min_diag=0.5):
    ...
    try:
        ...
        reweight = np.clip(reweight, 0.5, 2.0)  
